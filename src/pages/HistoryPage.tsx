        ...
        <div className={`noc-panel ${rollbackMutation.data.success ? 'border-success/30' : 'border-destructive/30'}`}>
          ...
          {rollbackMutation.data.restoredFiles.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {rollbackMutation.data.restoredFiles.map(f => (
      ...
      {showBackups && backups && backups.length > 0 && (
        <div className="noc-panel">
          ...
          <div className="space-y-2">
            {backups.map(b => (
              <div key={b.backupId} className="flex items-center gap-3 p-2 rounded bg-secondary border border-border text-xs">
                <Shield size={12} className="text-accent shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="font-mono truncate">{b.backupId}</div>
                  <div className="text-muted-foreground">{safeDate(b.timestamp)} · {b.operator} · {b.fileCount} arquivos</div>
                </div>
                <button onClick={() => handleRollback(b.backupId)} disabled={rollbackMutation.isPending}
            ...
            <div key={h.id} className="noc-panel">
              <div className="flex items-center justify-between mb-1 cursor-pointer" onClick={() => setExpandedId(expanded ? null : h.id)}>
                <div className="flex items-center gap-3">
                  ...
                  {h.configVersion && (
                    <span className="text-xs font-mono text-muted-foreground">{h.configVersion}</span>
                  )}
                </div>
                <div className="flex items-center gap-3">
              ...
              {expanded && (
                <div className="space-y-3 mt-3 border-t border-border pt-3">
                  ...
                  {/* Changed files */}
                  {h.changedFiles && h.changedFiles.length > 0 && (
                    <div>
