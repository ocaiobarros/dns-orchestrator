    ...
    <div className="space-y-6">
      ...
      <div className="noc-panel">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 text-sm">
          ...
          {/* VIP Anycast with explicit status */}
          <div className="flex justify-between py-1 border-b border-border">
            <span className="text-muted-foreground">VIP Anycast</span>
            <span className="font-mono text-foreground">
              {sysInfo?.vip_anycast_available
                ? (sysInfo?.vip_anycast || '—')
                : <span className="text-xs text-muted-foreground italic">Não configurado</span>}
            </span>
          </div>
          {/* Config Version with explicit status */}
          <div className="flex justify-between py-1 border-b border-border">
            <span className="text-muted-foreground">Config Version</span>
            <span className="font-mono text-foreground">
              {sysInfo?.config_version_available
                ? (sysInfo?.config_version || '—')
                : <span className="text-xs text-muted-foreground italic">Sem versão aplicada</span>}
            </span>
          </div>
          {/* Last apply with explicit status */}
          <div className="flex justify-between py-1 border-b border-border last:border-0">
            ...
            <span className="font-mono text-foreground">
              {sysInfo?.last_apply_available
                ? safeDate(sysInfo?.last_apply_at ?? sysInfo?.lastApply)
                : <span className="text-xs text-muted-foreground italic">Nenhuma aplicação registrada</span>}
            </span>
          </div>
        </div>
      </div>
