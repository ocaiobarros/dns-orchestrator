...
export default function TopologyMini({
  ...
  return (
    <div className="relative flex items-center justify-around gap-2 py-4 px-1 min-h-[260px] h-full w-full min-w-0 overflow-hidden font-mono text-[10px]">
      ...
      <div className="flex flex-col items-center gap-1.5 flex-shrink min-w-0">
        <div className="px-3 py-2 rounded-lg bg-card border border-primary/40 text-center min-w-0 w-full max-w-[clamp(110px,12vw,180px)]"
          ...
          <div className="flex items-center justify-center gap-1 mt-1 min-w-0">
            ...
            {extraCount > 0 && (
              <span className="flex-shrink-0 px-1 rounded bg-primary/15 text-primary text-[9px] font-bold leading-tight">
                +{extraCount}
              </span>
            )}
          </div>
          <div className="text-muted-foreground text-[9px] mt-1">{frontendQps} q/s</div>
        </div>
      </div>
      ...
      {/* Backends */}
      <div className="flex flex-col gap-2 flex-shrink min-w-0 justify-around h-full py-1">
        {backends.length === 0 && (
        ...
        {backends.slice(0, 4).map((b) => {
          ...
          return (
            <div key={b.name} className="px-2.5 py-1 rounded-md bg-card border min-w-0 w-full max-w-[clamp(110px,12vw,180px)]"
              style={{
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-[10px] font-bold ${ok ? 'text-primary' : 'text-destructive'}`}>{b.name}</span>
              </div>
              <div className="text-foreground/90 text-[10px] font-mono truncate">{b.ip}</div>
      ...
      {/* Upstream */}
      <div className="flex flex-col items-center gap-2 flex-shrink-0">
        <div className="w-16 h-16 rounded-lg bg-warning/10 border border-warning/40 flex items-center justify-center"
          style={{ boxShadow: '0 0 14px -5px hsl(var(--warning) / 0.5)' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--warning))" strokeWidth="1.6">
