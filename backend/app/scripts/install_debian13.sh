...
COLLECTOR_SVC_SRC="${INSTALL_DIR}/deploy/systemd/dns-control-collector.service"
COLLECTOR_TMR_SRC="${INSTALL_DIR}/deploy/systemd/dns-control-collector.timer"
if [ -f "$COLLECTOR_SVC_SRC" ] && [ -f "$COLLECTOR_TMR_SRC" ]; then
    ...
    fail "Collector systemd units not found at ${COLLECTOR_SVC_SRC}"
    ERRORS=$((ERRORS+1))
fi
# ── Install collector script ──
mkdir -p "${INSTALL_DIR}/collector"
if [ -f "${INSTALL_DIR}/backend/collector/collector.py" ]; then
    cp "${INSTALL_DIR}/backend/collector/collector.py" "${INSTALL_DIR}/collector/collector.py"
    cp "${INSTALL_DIR}/backend/collector/config.json" "${INSTALL_DIR}/collector/config.json" 2>/dev/null || true
    ok "Collector script installed to ${INSTALL_DIR}/collector/"
else
    fail "Collector script not found at ${INSTALL_DIR}/backend/collector/collector.py"
    ERRORS=$((ERRORS+1))
fi
