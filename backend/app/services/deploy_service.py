...
def _execute_deploy_locked(
    ...
    def selective_cleanup():
        ...
        # Files NEVER cleaned even when matching wizard globs.
        # /etc/unbound/unbound-block-domains.conf and anablock.conf are
        # auxiliary include placeholders that must always exist (otherwise
        # Unbound fails to load). They are regenerated by the deploy itself,
        # so removing them mid-cleanup is unnecessary and risky.
        _CLEANUP_PROTECTED = {
            "/etc/unbound/unbound-block-domains.conf",
            "/etc/unbound/anablock.conf",
            "/etc/unbound/block-domains.txt",
            "/etc/unbound/gen-block-domains.sh",
            "/etc/unbound/named.cache",
        }
        # Strict instance-name regex: only unboundNN.conf is wizard-generated.
        # Skip unbound-block-domains.conf and other includes that match the glob.
        _INSTANCE_CONF_RE = re.compile(r"^unbound\d+\.conf$")
        ...
        for label, pattern in _CLEANUP_GLOBS.items():
            ...
            for fpath in matched:
                if fpath in _NEVER_TOUCH or fpath in _CLEANUP_PROTECTED:
                    continue
                # Extra strict filter for unbound config glob
                if label == "unbound_configs":
                    basename = os.path.basename(fpath)
                    if not _INSTANCE_CONF_RE.match(basename):
                        continue
                try:
                    os.remove(fpath)
                    removed += 1
